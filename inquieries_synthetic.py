...
for entity in entities:
    ...
    for speech in text:
        for phrase in speech:
            ...
            n = 0
            for i, wrd in enumerate(sentence[2:]):
                prev = sentence[i-2]
                curr = sentence[i-1]
                next = sentence[i]
                #print(prev)
                #print(next)
                if curr['ana'] == '#xx' and curr['gotcommas'] == False:
                    #print(prev)
                    #print(next)
                    if prev['ana'] in tags and prev['lemma'] == 'do':
                        if next['ana'] in tags and next['gotcommas'] == False:
                            #print(prev)
                            n += 1
                            collocation = prev['word']+' '+curr['word']+' '+next['word']
                            ind += 1
            if n > 0:
                entries[phrase] = sentence
                phrase = re.sub(',', '<comma>', phrase)
                line = str(ind) + ',' + phrase + ',' + collocation + '\n'
                with open('entries_synthetic.tsv', 'a', encoding='utf-8') as f:
                    f.write(line)
                #if n > 1:
                #    quantity[n] = sentence
                #print(sentence)
